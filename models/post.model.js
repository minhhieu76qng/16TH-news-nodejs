...
module.exports = {
    all: () => {
        return db.load('select * from post where is_deleted=0 and id_status=2');
    },

    newest: (limit) => {
        if (limit <= 0) {
            return db.load(`select * from post where is_deleted=0 and id_status=2 order by date_posted desc`);
        } else {
            return db.load(`select * from post where is_deleted=0 and id_status=2 order by date_posted desc limit ${limit}`);
        }
    },
    ...
    newest_Thumbnail: (limit) => {
        if (limit <= 0) {
            return db.load(`select p.id, p.title, p.date_posted, p.cover_image, p.type_post, p.id_category, c.cat_name 
                from post p, category c where p.is_deleted=0 and p.id_status=2 and c.is_deleted=0 and p.id_category = c.id 
    ...
    countByCat : CatId => {
        return db.load(`select count(*) as total from post where is_deleted=0 and id_status=2 and id_category = ${CatId}`);
    },
    ...
    countByRootCat : (CatId) => {
        return db.load(`select count(*) as total from post p, category c 
            where p.is_deleted=0 and p.id_status=2 and c.parent_cat = ${CatId} and p.id_category = c.id`);
    },

    newestByCat: (CatId, limit) => {
        if (limit <= 0) {
            return db.load(`select * from post where is_deleted=0 and id_status=2 and id_category = ${CatId} order by date_posted desc`);
        } else {
            return db.load(`select * from post where is_deleted=0 and id_status=2 and id_category = ${CatId} order by date_posted desc limit ${limit}`);
        }
    },
    ...
    mostView: (limit) => {
        if (limit <= 0) {
            return db.load(`select * from post where is_deleted=0 and id_status=2 order by views desc`);
        } else {
            return db.load(`select * from post where is_deleted=0 and id_status=2 order by views desc limit ${limit}`);
        }
    },
    ...
    mostViewInTime: (time, limit) => {
        if (limit <= 0) {
            return db.load(`select * from post where is_deleted=0 and id_status=2 and date_posted between '${time.start}' and '${time.end}' order by views desc`);
        } else {
            return db.load(`select * from post where is_deleted=0 and id_status=2 and date_posted between '${time.start}' and '${time.end}' order by views desc limit ${limit}`);
        }
    },
    ...
    countByTag : TagID => {
        return db.load(`select count(*) as total from post p, post_tag pt where p.id=pt.id_post and p.is_deleted=0 and p.id_status=2
            and pt.id_tag=${TagID}`);
    },
